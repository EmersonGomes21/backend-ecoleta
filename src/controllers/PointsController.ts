...
class PointsController{
  async index(request:Request, response:Response){
    const {
      ...
      const parsedItems =
       String(items)
       .split(',')
       .map(item => Number(item.trim())); //percorrendo o array e procurando pelas virgulas e removendo-as da right and left / forçando o retorno a numero  
  ...
  async create(request: Request, response: Response){
    const {
     ...
     const point_id = insertedIds[0]; //posição zero é o id
